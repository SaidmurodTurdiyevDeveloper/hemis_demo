...
List<StudentSubjectTasksData> createDefaultTaskList() {
  final  trainingType = StudentSubjectTasksTrainingType(code: 'LAB', name: 'Laboratoriya');
  final taskType     = StudentSubjectTasksTrainingType(code: 'HW',  name: 'Uyga vazifa');
  final taskStatus   = StudentSubjectTasksTrainingType(code: 'NEW', name: 'Yangi');

  final now = DateTime.now();
  final List<StudentSubjectTasksData> list = [];
  ...
  for (int i = 1; i <= 5; i++) {
    list.add(
      StudentSubjectTasksData(
        id: i,
        name: 'Task $i',
        comment: 'Default izoh $i',
        maxBall: 100,
        deadline: now.add(Duration(days: i * 3)).millisecondsSinceEpoch ~/ 1000, // sekund
        attemptLimit: 3,
        trainingType: trainingType,
        taskType: taskType,
        taskStatus: taskStatus,
        employee: StudentSubjectTasksEmployee(
          id: 100 + i,
          name: 'O‘qituvchi $i',
        ),
        files: [
          StudentSubjectTasksFiles(
            name: 'template_$i.pdf',
            size: 23456,
            url: 'https://example.com/template_$i.pdf',
          ),
        ],
        studentTaskActivity: StudentSubjectTasksStudentTaskActivity(
          id: 200 + i,
          comment: 'Hali topshirilmagan',
          mark: null,
          files: [],
        ),
